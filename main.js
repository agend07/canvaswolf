...
(function() {
  var Hero, Hit, Line, Point, Wall, buf8, canvasHeight, canvasWidth, cast, clear, ctx, data, dec, draw, drawGradient, drawTexture, fps, fpsSpan, getDecimal, getTextureColor, hero, imageData, initialFrame, keys, loadTexture, pcast, project, render, round3, run, settings, sizeX, textureCanvas, textureCanvasHeight, textureCanvasWidth, textureData, world;
  ...
  Line = class Line {
    constructor(a1 = new Point(), b1 = new Point()) {
      this.a = a1;
      this.b = b1;
    }
  ...
  setInterval((function() {
    return fpsSpan.innerHTML = round3(fps);
  }), 200);

  getDecimal = function(point) {
    var decX;
    decX = dec(point.x);
    if (decX > 0) {
      return decX;
  ...
  render = function(start) {
    var camera, column, corrected, height, hit, i, j, k, l, lastTheta, lastWhere, ms, ray, ref, ref1, ref2, ref3, ref4, ref5, sel, stop, tile, trace, uCord, vCord, wall, x, y;
    lastWhere = hero.where;
    lastTheta = hero.theta;
    hero.spin();
    hero.move();
    if (lastTheta !== hero.theta || lastWhere.x !== hero.where.x || lastWhere.y !== hero.where.y || initialFrame) {
      initialFrame = false;
...
// wall
        for (y = k = ref2 = wall.top, ref3 = wall.bot; (ref2 <= ref3 ? k < ref3 : k > ref3); y = ref2 <= ref3 ? ++k : --k) {
          ...
          switch (document.getElementById('wall').selectedIndex) {
            case 0:
              ...
              break;
            case 1:
              drawGradient(x, y, hit.Tile, uCord, vCord);
              break;
            case 2:
              drawTexture(x, y, hit.Tile, uCord, vCord);
          }
        }
        // flooring
  ...
  drawTexture = function(x, y, tile, uCord, vCord) {
    var a, b, color, g, idx, r;
  ...
  drawGradient = function(x, y, tile, uCord, vCord) {
    var blue, color, green, idx, red;
  ...
  clear = function() {
    ...
    results = [];
    for (idx = i = 0, ref = canvasHeight * canvasWidth; (0 <= ref ? i < ref : i > ref); idx = 0 <= ref ? ++i : --i) {
      results.push(data[idx] = 0xFFAAFFAA);
    }
    return results;
  };
